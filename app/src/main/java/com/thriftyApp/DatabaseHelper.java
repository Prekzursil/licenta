...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public Contact getUserByGoogleId(String googleUserId) {
        ...
        if (cursor.moveToFirst()) {
            // Check that the column index is valid before accessing it
    ...
    public List<String> searchUserId(String googleUserId) {
        ...
        if (cursor != null && cursor.moveToFirst()) {
            // If user exists, retrieve the details
            id = cursor.getString(0);        // User ID
            String email = cursor.getString(1);
            pass = cursor.getString(2);      // Password
            Utils.userName = cursor.getString(3);  // Name
            budget = cursor.getString(4);    // Budget

            Log.i("User Details", "ID: " + id + ", Name: " + Utils.userName);

            // Add the retrieved details to the list
            list.add(pass);
            list.add(id);
            list.add(budget);
        }
